...
// 1. Find all items in the current directory (your "testing" root folder)
...
folders.forEach(folder => {
    ...
    if (folderBugCount > 0) {
        // This will log the total number of descriptions extracted from this repo
        // E.g., if a repo has 4 bugs, it will extract 12 descriptions total (4 bugs * 3 files)
        console.log(`  ✓ Extracted ${folderBugCount} bug descriptions from ${folder}`);
    } else {
        console.log(`  ! No testing JSONs found in ${folder}`);
    }
});

// 4. Write everything to a single testing.json file in the root directory
const outputPath = path.join(__dirname, 'testing.json');
fs.writeFileSync(outputPath, JSON.stringify(allBugs, null, 4), 'utf8');

console.log(`\n🎉 Successfully compiled ${allBugs.length} total bug entries into testing.json!`);
