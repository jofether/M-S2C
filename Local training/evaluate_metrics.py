...
def evaluate_model(sample_limit=100):
    """
    Evaluates the model on the dataset and calculates Top-K, MRR, and MAP.
    
    Args:
        sample_limit (int): How many queries to test (use a smaller number for a quick test).
    """
    logger.info("==================================================")
    logger.info("🚀 INITIALIZING M-S2C COMPONENT-BASED VALIDATION")
    logger.info("==================================================\n")
    ...
    if os.path.exists(weights_path):
        ...
        logger.warning("⚠️ Phase 3 weights not found! Running with untrained model.")
    model.eval()
    ...
    # Use the last N samples as our "Test Set" to avoid testing on training data
    test_set = real_data[-sample_limit:] 
    ...
    for i, sample in enumerate(test_set):
        ...
        image_path = os.path.join(REPO_DIR, "data", "screenshots", sample['image_anchor'])
        ground_truth_code = sample['positive_node'].strip()

        # Process Modalities
        text_enc = tokenizer(query_text, padding='max_length', truncation=True, max_length=128, return_tensors="pt").to(device)
...
if __name__ == "__main__":
    # You can change sample_limit to a higher number to test the full dataset
    evaluate_model(sample_limit=100)